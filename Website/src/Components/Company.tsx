...
const Company: React.FC = () => {
  return (
    <div className="w-full py-16 flex flex-col bg-[#FDFDFD] justify-center items-center">
      <h1 className="text-3xl px-4 font-semibold text-center">Trusted by Top Companies</h1>
      <div className="flex flex-wrap justify-center gap-6 px-6 mt-10">
        {logos.map((logo, index) => (
          <img key={index} src={logo.src} className="w-28 h-24 md:w-36 md:h-32 lg:w-48 lg:h-40 object-contain" alt={logo.alt} />
        ))}
      </div>
    </div>
  );
